...
$targetProcs = @("7za", "aria2c", "git", "python", "pip", "node")
Get-Process -Name $targetProcs -ErrorAction SilentlyContinue | Where-Object { 
    $_.Path -and $_.Path.StartsWith($root, [System.StringComparison]::InvariantCultureIgnoreCase) 
...
function Get-PipProxyArgs {
    ...
    param ()
    if (Test-ProxyHealth) {
        return @("--trusted-host", ([uri]$ProxyServerUrl).Host, "--index-url", "$ProxyServerUrl/pypi/simple/")
    }
    return @()
}
...
function Install-Python {
    ...
    try {
        ...
        if (-not (Test-Path (Join-Path $pipDir "pip.exe"))) {
            ...
            $pipArgs = @($getPipScript, "--no-warn-script-location") + (Get-PipProxyArgs)
            Invoke-SetupCommand -CommandPath $pyExe -ArgumentList $pipArgs
        }
        else {
            $pipArgs = @("-m", "pip", "install", "--upgrade", "pip") + (Get-PipProxyArgs)
...
function Enter-LiveDevShell {
    ...
    if (Test-Path $pyExe) {
        if (Test-ProxyHealth) {
            Write-SetupStatus -Message "[PROXY] Auto-configuring pip to use OmniProxy Gateway..." -Type INFO
            Invoke-SetupCommand -CommandPath $pyExe -ArgumentList @("-m", "pip", "config", "set", "global.index-url", "$ProxyServerUrl/pypi/simple/") -NoOutput
            Invoke-SetupCommand -CommandPath $pyExe -ArgumentList @("-m", "pip", "config", "set", "global.trusted-host", ([uri]$ProxyServerUrl).Host) -NoOutput
        } else {
            Invoke-SetupCommand -CommandPath $pyExe -ArgumentList @("-m", "pip", "config", "unset", "global.index-url") -NoOutput
            Invoke-SetupCommand -CommandPath $pyExe -ArgumentList @("-m", "pip", "config", "unset", "global.trusted-host") -NoOutput
        }
    }
...
function Show-HelpMenu {
    ...
    Write-Host "   -p, --proxy      Override proxy (e.g. -p disable, -p 10.0.0.5:8080)"
